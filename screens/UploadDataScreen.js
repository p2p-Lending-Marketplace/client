...
import { UPLOAD_IMAGE, UPDATE_USER_DATA, FETCH_USER_DETAIL } from '../API/graphQuery'
import colors from "../assets/colors"
import { APP_NAME } from '../assets/variables'
import { PersonalDataComponent, FinancialDataComponent, SegmentUploadComponent } from "../components"
...
const UploadDataScreen = ({ navigation }) => {
  ...
  const [updateUserData, { data: updateData, error: updateError, loading: updateLoding }] = useMutation(UPDATE_USER_DATA)
    ...
    <SafeAreaView style={{ flex: 1, backgroundColor: colors.whiteBackground }}>
      ...
      <ScrollView style={{ flex: 1 }} showsVerticalScrollIndicator={false}>
        {personal && dataUser._id ? (
          <PersonalDataComponent data={{user: dataUser, handleSaveButton, handleDataChange, datePicker, datepicker, _pickImage, setDate, photoData, photoLoading, idCardData, idCardLoading}} />
        ) : financial && dataUser._id ? (
          <FinancialDataComponent data={{user: dataUser, handleSaveButton, handleDataChange, _pickImage, salarySlipData, salarySlipLoading}} />
        ) : null}
      </ScrollView>
    </SafeAreaView>
  )
