...
import { View, Text, SafeAreaView, Image, TouchableOpacity, AsyncStorage } from "react-native";
...
const RegisterScreen = ({parentNavigation}) => {
  ...
  const [verifyOTP, { loading: verifyLoading, error: verifyError, data: verifyData }] = useLazyQuery(VERIFY_OTP)
  ...
  const validationRegister = async () => {
    if(phoneNumber.length > 9 && otpCode.length === 6) await setValidation(true)
            ...
            <Item style={{ width: '90%' }}>
              ...
              />
              {
                resendStatus
                  ? <CountDown
                      until={60}
                      style={{borderColor: 'grey', borderWidth: 1, borderRadius: 3, padding: 2}}
